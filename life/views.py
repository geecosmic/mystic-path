...
def all_period(request):
    ...
    # ------------------------------------------------
    # Get current time for the top display
    now = datetime.now()

    # Define the length of each period in seconds
    period_length_in_seconds = (24 * 60 * 60) // 7  # Total seconds in a day divided by 7
    ...
    # Processing the user search for a specific day and time
    period_letter = None
    day_input = None
    time_input = None

    if request.method == "POST" and 'time' in request.POST:
        # Get day and time input from the form
        day_input = request.POST.get('day').title()
        time_input = request.POST.get('time')

        # Convert time_input to a datetime object
        try:
            time_obj = datetime.strptime(time_input, '%H:%M')
        except ValueError:
            time_obj = datetime.now()  # Fallback to current time if time is invalid

        # Get the period sequence for the input day
        periods = period_mappings.get(day_input, None)

        if periods:
            # Calculate elapsed time from midnight of the input day
            time_since_midnight = time_obj.hour * 3600 + time_obj.minute * 60  # Seconds since midnight

            # Determine which period the input time falls into
            period_index = int(time_since_midnight // period_length_in_seconds)

            if 0 <= period_index < len(periods):
                period_letter = periods[period_index]
            else:
                period_letter = "Invalid period"
        else:
            period_letter = "Invalid day"
    ...
    if request.method == "POST" and 'month' in request.POST:
        # Get the user inputs for the yearly cycle
        month_input = request.POST.get('month').title()
        date_input = request.POST.get('date')
...
# -------------------jewish birthday-----------------------------
    ...
    return render(request, 'all_period.html', context)
...
from django.shortcuts import render
from .utils import hebrew_date_info
